...
const RepertoryPage = () => {
  ...
  const sourceTest = () => {
    ...
    formData.append('postSource', new Blob([JSON.stringify(data)], {type: "application/json"}));
    dance.postSource(formData);
  }

  const repertoryTest = () => {
    const url = `${BASE_URL}/dance/repertory`;
    ...
    formData.append('postRepertoryRequest', new Blob([JSON.stringify(data)], {type: "application/json"}));
  ...
  return (
    <>
      ...
      <Btn onClick={()=>dance.searchSourceName('hi')}>POST/searchSourceName</Btn>
      <Btn onClick={()=>dance.fetchSourceInfo(123)}>POST/fetchSourceInfo</Btn>
